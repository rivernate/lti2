...
module LTI2::Models
  describe BaseUrlChoice do
    ...
    describe '#default_message_path' do
      ...
      it 'it returns the secure message base' do
        ...
        expect(subject.default_message_path).to eq 'http://secure.com/path'
      end
      ...
      it 'returns the default message base if there is no secure path' do
        ...
        subject.selector = selector
        expect(subject.default_message_path).to eq 'http://base.com/path'
      end

      it 'returns an empty string if there is no base url' do
        expect(subject.default_message_path).to eq ''
      end
